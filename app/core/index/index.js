// app/core/node/index/index.js

class CoreNodeIndex {
    constructor(baseInstance) {
        this.base = baseInstance;
        this.toolbar = new CoreNodeIndexToolbar(this);
        this.cards = new CoreNodeIndexCards(this);
    }

    async openModules() {
        // Динамически загружаем CSS модуля
        if (!document.querySelector('link[data-module="modules"]')) {
            const cssUrl = `/static/core/node/index/modules/modules.css?v=${window.APP_VERSION}`
        ...
        try {
            const jsUrl = `/static/core/node/index/modules/modules.js?v=${window.APP_VERSION}`
            await import(jsUrl);
            const modulesModal = new CoreNodeIndexModules({
                onSave: async (data) => {
                    console.log('Модули сохранены:', data);
                    // Можно обновить что-то на странице
                }
            });
            modulesModal.show();
        } catch (error) {
    ...
    showMessage(message, type = 'success') {
        ...
        let container = document.querySelector('.core-node-messages');
        
        if (!container) {
            const subheader = document.querySelector('.core-node-base-subheader');
            if (!subheader) return;
            
            container = document.createElement('div');
            container.className = 'core-node-messages';
...
document.addEventListener('DOMContentLoaded', () => {
    new CoreNodeIndex(window.coreNodeBase);
});
