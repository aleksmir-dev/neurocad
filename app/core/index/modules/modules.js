// app/core/node/index/modules/modules.js

class CoreNodeIndexModules {
    constructor(options = {}) {
    ...
    async loadHtml() {
        if (!document.querySelector('.core-node-index-modules')) {
            const response = await fetch('/core/node/index/modules/modal');
            const html = await response.text();
            document.body.insertAdjacentHTML('beforeend', html);
        }
        this.modal = document.querySelector('.core-node-index-modules');
        this.editModal = document.querySelector('.core-node-index-modules-edit');
        this.msgBox = document.querySelector('.core-node-index-modules-msgbox');
        this.confirmBox = document.querySelector('.core-node-index-modules-confirm');
    }
    ...
    async loadList() {
        ...
        const url = this.showDeleted ? '/core/node/index/modules/list?include_deleted=true' : '/core/node/index/modules/list';
        try {
    ...
    async openEdit(id = null) {
        ...
        if (id) {
            ...
            try {
                const res = await fetch(`/core/node/index/modules/module/${id}`);
                const data = await res.json();
    ...
    async save() {
        ...
        try {
            let res;
            if (id) {
                res = await fetch(`/core/node/index/modules/module/${id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name, url, description: desc }) });
            } else {
                res = await fetch('/core/node/index/modules/module', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name, url, description: desc }) });
            }
    ...
    async deleteSelected() {
        ...
        this.confirm(`Удалить ${this.selectedIds.size} модуль(ей)?`, async (ok) => {
            ...
            for (const id of this.selectedIds) {
                const res = await fetch(`/core/node/index/modules/module/${id}`, { method: 'DELETE' });
                if (res.ok) success++;
            }
    ...
    async restoreSelected() {
        ...
        this.confirm(`Восстановить ${this.selectedIds.size} модуль(ей)?`, async (ok) => {
            ...
            let success = 0;
            for (const id of this.selectedIds) {
                const res = await fetch(`/core/node/index/modules/module/${id}/restore`, { method: 'POST' });
                if (res.ok) success++;
            }
            this.selectedIds.clear();
            await this.loadList();
...
window.CoreNodeIndexModules = CoreNodeIndexModules;